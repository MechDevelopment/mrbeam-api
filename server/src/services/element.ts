import { ElemLength, INode } from "./../core/calculate.core";
...
export function getDistInCoord(dist: IDist, x: number): number {
  const [x1, x2] = dist.x;
...
export class Elem {
  public distload: Array<IDist> = [];
  public material: [number, number, number] = [1, 1, 1];

  constructor(public nodes: [INode, INode]) {}
  ...
  setDistload(distload: Array<IDist>) {
  ...
  // static
  static getLength(elems: Array<Elem>): ElemLength {
    const start = elems
      .map((el) => el.nodes[0].x)
