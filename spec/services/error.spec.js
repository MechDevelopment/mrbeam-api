...
describe('handleError function', () => {
  ...
  it(`should be error <${exeption5}> if units have bad <x> property`, () => {
    ...
    expect(() =>
      handleErrors([{ x: [0, 4], type: 'simple' }])
    ).not.toThrowError(exeption5)
    expect(() =>
      handleErrors([{ x: new Date(), type: 'simple' }])
    ).not.toThrowError(exeption5)

    expect(() => handleErrors([{ x: 's12', type: 'simple' }])).toThrowError(
      exeption5
    )
    expect(() => handleErrors([{ x: { x: 1 }, type: 'simple' }])).toThrowError(
      exeption5
    )
  })
  ...
  it(`should be error <${exeption6}> if units have bad <type> property`, () => {
    ...
    expect(() => handleErrors([{ x: 0, type: 'distload' }])).not.toThrowError(
      exeption6
    )

    expect(() => handleErrors([{ x: 0, type: 'simpleX' }])).toThrowError(
      exeption6
    )
    expect(() => handleErrors([{ x: 0, type: 'distloadX' }])).toThrowError(
      exeption6
    )
  })
  ...
  it(`should be error <${exeption7}> if units havent <value> property`, () => {
    ...
    expect(() => handleErrors([{ x: 0, type: 'hinge' }])).not.toThrowError(
      exeption7
    )

    expect(() => handleErrors([{ x: 0, type: 'force' }])).toThrowError(
      exeption7
    )
    expect(() => handleErrors([{ x: 0, type: 'distload' }])).toThrowError(
      exeption7
    )
  })
  ...
  it(`should be error <${exeption8}> if units have bad <value> property`, () => {
    ...
    expect(() =>
      handleErrors([{ x: 0, type: 'force', value: 12 }])
    ).not.toThrowError(exeption8)
    expect(() =>
      handleErrors([{ x: 0, type: 'force', value: '12' }])
    ).not.toThrowError(exeption8)
    expect(() =>
      handleErrors([{ x: 0, type: 'distload', value: [12, 21] }])
    ).not.toThrowError(exeption8)

    expect(() =>
      handleErrors([{ x: 0, type: 'force', value: 's12' }])
    ).toThrowError(exeption8)
    expect(() =>
      handleErrors([{ x: 0, type: 'force', value: undefined }])
    ).toThrowError(exeption8)
  })
  ...
  it(`should be error <${exeption9}> if units count is 0 or 1`, () => {
    ...
    expect(() => handleErrors([{ x: 0, type: 'fixed' }])).toThrowError(
      exeption9
    )
  })
